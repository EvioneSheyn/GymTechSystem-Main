...
router.post("/finish-exercise", auth, async (req, res) => {
  ...
  try {
    await WorkoutSession.create({ userId, routineId, caloriesBurned, duration });
    return res.status(200).json({ message: "Workout recording complete!" });
  } catch (error) {
    return res.status(500).json({ message: "Error recording workout", error: error.message });
  }
});
...
router.get("/all", auth, async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: "Successfully retrieved workout sessions", sessions });
  } catch (error) {
...
router.post("/by-date", auth, async (req, res) => {
  ...
  try {
    ...
    const totalCaloriesBurned = workoutSessions.reduce((acc, s) => acc + Number(s.caloriesBurned || 0), 0);

    return res.status(200).json({ message: "Successfully retrieved calories-burned record!", sessions: workoutSessions, caloriesBurned: totalCaloriesBurned });
  } catch (error) {
    return res.status(500).json({ message: "Error retrieving calories burned", error: error.message });
  }
});
